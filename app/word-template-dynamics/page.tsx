...
const platforms = ["全部", "觅知手抄报", "觅知营销日历", "熊猫办公"]
...
export default function WordTemplateDynamicsPage() {
  ...
  const filteredTemplates = allTemplates.filter((template) => {
    ...
    // Type filter
    if (selectedType !== "全部" && template.type !== selectedType) return false
    
    // Category filter
    if (selectedCategory !== "全部" && template.category !== selectedCategory) return false
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
          <h1 className="mb-2 text-2xl font-bold text-foreground">文字模板动态</h1>
          <p className="text-sm text-muted-foreground">实时追踪手抄报、营销素材等文字模板动态，把握教育与营销节点</p>
          </div>
          <Button
            onClick={() => {
        ...
        <div className="mb-6 rounded-xl bg-card p-5 shadow-sm border">
          ...
          {/* Type Filter */}
          <div>
            <label className="mb-2 block text-xs font-medium text-muted-foreground">类型:</label>
            <div className="flex flex-wrap gap-2">
              {types.map((type) => (
                <Button
                  key={type}
